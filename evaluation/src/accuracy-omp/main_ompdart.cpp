...
int main(int argc, char* argv[])
{
  ...
  #pragma omp target data map(alloc:count) map(to:label,data)
  {
    for (int ngrid = nrows / 4; ngrid <= nrows; ngrid += nrows / 4) {
      ...
      for (int i = 0; i < repeat; i++) {
        count[0] = 0;
        #pragma omp target update to(count)
        // #pragma omp target update to (count[0:1]) 
  
        #pragma omp target teams distribute num_teams(ngrid) firstprivate(nrows,ndims,top_k)
      ...
      #pragma omp target update from(count)
