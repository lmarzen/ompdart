...
{
  //#pragma omp target teams distribute parallel for collapse(3) thread_limit(256) map(to:phi[0:DATAXSIZE*DATAYSIZE*DATAZSIZE]) map(from:Fx[0:DATAXSIZE*DATAYSIZE*DATAZSIZE],Fy[0:DATAXSIZE*DATAYSIZE*DATAZSIZE],Fz[0:DATAXSIZE*DATAYSIZE*DATAZSIZE]) firstprivate(dx,dy,dz,epsilon,W0)
  #pragma omp target teams distribute parallel for collapse(3) thread_limit(256) map(to:phi[0:DATAXSIZE]) map(from:Fx[0:DATAXSIZE],Fy[0:DATAXSIZE],Fz[0:DATAXSIZE]) firstprivate(dx,dy,dz,epsilon,W0)
...
{
  #pragma omp target teams distribute parallel for collapse(3) thread_limit(256) map(to:phiold[0:DATAXSIZE],uold[0:DATAXSIZE],Fx[0:DATAXSIZE],Fy[0:DATAXSIZE],Fz[0:DATAXSIZE]) map(from:phinew[0:DATAXSIZE]) firstprivate(epsilon,tau0,lambda,dt,dx,dy,dz)
  //#pragma omp target teams distribute parallel for collapse(3) thread_limit(256) map(to:phiold[0:DATAXSIZE*DATAYSIZE*DATAZSIZE],uold[0:DATAXSIZE*DATAYSIZE*DATAZSIZE],Fx[0:DATAXSIZE*DATAYSIZE*DATAZSIZE],Fy[0:DATAXSIZE*DATAYSIZE*DATAZSIZE],Fz[0:DATAXSIZE*DATAYSIZE*DATAZSIZE]) map(from:phinew[0:DATAXSIZE*DATAYSIZE*DATAZSIZE]) firstprivate(epsilon,tau0,lambda,dt,dx,dy,dz)
...
{
  #pragma omp target teams distribute parallel for collapse(3) thread_limit(256) map(from:phinew[0:DATAXSIZE])
  //#pragma omp target teams distribute parallel for collapse(3) thread_limit(256) map(from:phinew[0:DATAXSIZE*DATAYSIZE*DATAZSIZE])
  for (int ix = 0; ix < DATAXSIZE; ix++) {
    for (int iy = 0; iy < DATAYSIZE; iy++) {
      for (int iz = 0; iz < DATAZSIZE; iz++) {
...
{
  #pragma omp target teams distribute parallel for collapse(3) thread_limit(256) map(to:uold[0:DATAXSIZE],phinew[0:DATAXSIZE],phiold[0:DATAXSIZE]) map(from:unew[0:DATAXSIZE]) firstprivate(D,dt,dx,dy,dz)
  //#pragma omp target teams distribute parallel for collapse(3) thread_limit(256) map(to:uold[0:DATAXSIZE*DATAYSIZE*DATAZSIZE],phinew[0:DATAXSIZE*DATAYSIZE*DATAZSIZE],phiold[0:DATAXSIZE*DATAYSIZE*DATAZSIZE]) map(from:unew[0:DATAXSIZE*DATAYSIZE*DATAZSIZE]) firstprivate(D,dt,dx,dy,dz)
  for (int ix = 1; ix < DATAXSIZE-1; ix++) {
    for (int iy = 1; iy < DATAYSIZE-1; iy++) {
      for (int iz = 1; iz < DATAZSIZE-1; iz++) {
...
{
  #pragma omp target teams distribute parallel for collapse(3) thread_limit(256) map(from:unew[0:DATAXSIZE]) firstprivate(delta)
  //#pragma omp target teams distribute parallel for collapse(3) thread_limit(256) map(from:unew[0:DATAXSIZE*DATAYSIZE*DATAZSIZE]) firstprivate(delta)
  for (int ix = 0; ix < DATAXSIZE; ix++) {
    for (int iy = 0; iy < DATAYSIZE; iy++) {
      for (int iz = 0; iz < DATAZSIZE; iz++) {
...
{
  #pragma omp target teams distribute parallel for collapse(3) thread_limit(256) map(tofrom:cnew[0:DATAXSIZE],cold[0:DATAXSIZE])
  //#pragma omp target teams distribute parallel for collapse(3) thread_limit(256) map(tofrom:cnew[0:DATAXSIZE*DATAYSIZE*DATAZSIZE],cold[0:DATAXSIZE*DATAYSIZE*DATAZSIZE])
  for (int ix = 0; ix < DATAXSIZE; ix++) {
    for (int iy = 0; iy < DATAYSIZE; iy++) {
      for (int iz = 0; iz < DATAZSIZE; iz++) {
