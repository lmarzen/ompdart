...
{
  #pragma omp target teams distribute parallel for collapse(3) thread_limit(256) firstprivate(dx,dy,dz,epsilon,W0)
...
{
  #pragma omp target teams distribute parallel for collapse(3) thread_limit(256) firstprivate(epsilon,tau0,lambda,dt,dx,dy,dz)
...
{
  #pragma omp target teams distribute parallel for collapse(3) thread_limit(256) firstprivate(D,dt,dx,dy,dz)
  for (int ix = 1; ix < DATAXSIZE-1; ix++) {
    for (int iy = 1; iy < DATAYSIZE-1; iy++) {
      for (int iz = 1; iz < DATAZSIZE-1; iz++) {
...
{
  #pragma omp target teams distribute parallel for collapse(3) thread_limit(256) firstprivate(delta)
  for (int ix = 0; ix < DATAXSIZE; ix++) {
    for (int iy = 0; iy < DATAYSIZE; iy++) {
      for (int iz = 0; iz < DATAZSIZE; iz++) {
...
int main(int argc, char *argv[])
{
  ...
  // {
    ...
    #pragma omp target data map(alloc:d_phinew[0:vol],d_unew[0:vol],d_Fx[0:vol],d_Fy[0:vol],d_Fz[0:vol]) map(tofrom:d_phiold[0:vol],d_uold[0:vol])
    {
      while (t <= num_steps) {
