...
{
  #pragma omp target teams distribute parallel for collapse(3) thread_limit(256) map(to:phi) map(from:Fx,Fy,Fz) firstprivate(dx,dy,dz,epsilon,W0)
...
{
  #pragma omp target teams distribute parallel for collapse(3) thread_limit(256) map(to:phiold,uold,Fx,Fy,Fz) map(from:phinew) firstprivate(epsilon,tau0,lambda,dt,dx,dy,dz)
...
{
  #pragma omp target teams distribute parallel for collapse(3) thread_limit(256) map(from:phinew)
  for (int ix = 0; ix < DATAXSIZE; ix++) {
    for (int iy = 0; iy < DATAYSIZE; iy++) {
      for (int iz = 0; iz < DATAZSIZE; iz++) {
...
{
  #pragma omp target teams distribute parallel for collapse(3) thread_limit(256) map(to:uold,phinew,phiold) map(from:unew) firstprivate(D,dt,dx,dy,dz)
  for (int ix = 1; ix < DATAXSIZE-1; ix++) {
    for (int iy = 1; iy < DATAYSIZE-1; iy++) {
      for (int iz = 1; iz < DATAZSIZE-1; iz++) {
...
{
  #pragma omp target teams distribute parallel for collapse(3) thread_limit(256) map(from:unew) firstprivate(delta)
  for (int ix = 0; ix < DATAXSIZE; ix++) {
    for (int iy = 0; iy < DATAYSIZE; iy++) {
      for (int iz = 0; iz < DATAZSIZE; iz++) {
...
{
  #pragma omp target teams distribute parallel for collapse(3) thread_limit(256) map(tofrom:cnew,cold)
  for (int ix = 0; ix < DATAXSIZE; ix++) {
    for (int iy = 0; iy < DATAYSIZE; iy++) {
      for (int iz = 0; iz < DATAZSIZE; iz++) {
