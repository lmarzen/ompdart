                           ...
                           int mype, double *kernel_time)
{
  ...
  #pragma omp target data map(alloc:in) map(to:SD_num_nucs[:SD.length_num_nucs],SD_concs[:SD.length_concs],SD_mats[:SD.length_mats],SD_nuclide_grid[:SD.length_nuclide_grid],SD_unionized_energy_array[:SD.length_unionized_energy_array],SD_index_grid[:SD.length_index_grid]) map(from:verification[:in.lookups])
  {

    double kstart = get_time();

    for (int n = 0; n < in.kernel_repeat; n++) {
      #pragma omp target update to(in)
  
      #pragma omp target teams distribute parallel for thread_limit(256) firstprivate(SD_max_num_nucs)
