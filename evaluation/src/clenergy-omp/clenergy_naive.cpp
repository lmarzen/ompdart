...
#pragma omp target enter data map(alloc: atominfo[0:MAXATOMS])
{
  #pragma omp target teams distribute parallel for simd map(tofrom: energy[0:volmem])
  ...
  for (atomstart=0; atomstart<atomcount; atomstart+=MAXATOMS) {   
    ...
    #pragma omp target teams distribute parallel for collapse(2) map(tofrom: energy[0:volmem]) // nowait
  ...
  #pragma omp target exit data map(delete:atominfo[0:MAXATOMS])
