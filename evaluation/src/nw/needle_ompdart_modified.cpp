        ...
        int max_rows, int max_cols, int penalty)
{
    ...
    #pragma omp target data map(to:referrence[0:transfer_size]) map(tofrom:input_itemsets[0:transfer_size])
    {
        #pragma omp target  firstprivate(max_rows,max_cols,penalty)
